...
cd /d "%~dp0.."
tar -cf - --exclude=node_modules --exclude=.next --exclude=.git --exclude=.github --exclude=.claude --exclude=memory-bank --exclude=docs --exclude=*.md --exclude=.env --exclude=.env.local --exclude=.dockerignore . | docker build -t site-roco:latest --build-arg NEXT_PUBLIC_MAUTIC_TRACKING_ENABLED=false --build-arg NEXT_PUBLIC_SITE_URL=http://localhost:3000 -
